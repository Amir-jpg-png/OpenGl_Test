...
bool GL_log_call(const char *function, const char *file, int line)
{
  ...
  {
    std::cout << "[OpenGL Error] " << GLErrorToString(error) << " " << function << " " << file << ":" << line << std::endl;
    return false;
  }
  return true;
}
...
void Renderer::set_clear_color(const glm::vec4 &clear_colors) const
{
  GLCall(glClearColor(clear_colors.x, clear_colors.y, clear_colors.z, clear_colors.w));
}
